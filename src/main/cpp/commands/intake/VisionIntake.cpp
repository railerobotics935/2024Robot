...
VisionIntake::VisionIntake(IntakeSubsystem* intake, StagerSubsystem* stager, DriveSubsystem* drive) {
  ...
  auto path = std::make_shared<PathPlannerPath>(
      ...
      GoalEndState(1.0_mps, m_drive->GetPose().Rotation()) // Goal end state. You can set a holonomic rotation here. If using a differential drivetrain, the rotation will have no effect.
  );
